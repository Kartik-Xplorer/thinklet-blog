...
function RecommendedPosts(props: Props) {
  const { recommendedPosts, post } = props;

  if (!recommendedPosts || recommendedPosts.length === 0) {
    return null;
  }

  const recommendedPostsRendered = recommendedPosts.map((postNode: any) => {
